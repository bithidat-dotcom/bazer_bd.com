import { Search, ShoppingBag, ShoppingCart, Filter as FilterIcon, X, Package, User, Tv, Volume, Volume1, Volume2, VolumeX } from 'lucide-react';
...
export default function Navbar({ 
            ...
            {[5, 25, 30, 50, 90].map(pct => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                  discountFilter === pct ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                {pct}% OFF
              </button>
        ...
        <div className="border-t border-slate-100 pt-4">
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
            ...
            <input 
              ...
              className="w-full accent-blue-600 h-2 cursor-pointer appearance-none outline-none focus:ring-1 focus:ring-blue-500 bg-slate-200 rounded-lg"
            />
          </div>
  ...
  return (
    <nav className="sticky top-0 z-50 bg-[#1E5EF3] px-4 py-3 sm:px-8 shadow-sm relative border-b-2 border-blue-700">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 relative">
        ...
        <div className="flex flex-1 md:max-w-lg relative gap-1 md:gap-2 min-w-0">
          <div className="relative flex-1 min-w-0">
            ...
            <input 
              ...
              className="w-full pl-9 md:pl-11 pr-3 md:pr-4 py-2 md:py-3 bg-white/10 rounded-xl md:rounded-full border border-white/20 focus:bg-white focus:ring-2 focus:ring-blue-400 focus:outline-none transition-all text-xs md:text-sm font-medium text-white placeholder:text-white/60 focus:text-slate-900"
            />
        ...
        <div className="hidden md:flex items-center gap-1 sm:gap-4 relative z-[100]">
          <button onClick={onTrackOrderClick} className="px-3 sm:px-4 py-2 hover:bg-blue-400/20 rounded-full transition-colors flex items-center gap-2 group text-white font-black text-sm">
            <Package className="w-5 h-5" />
            <span className="hidden sm:inline">My Products</span>
          </button>
          
          <button onClick={onCartClick} className="p-2 hover:bg-blue-400/20 rounded-full transition-colors relative group">
            <motion.div
              key={`cart-icon-${cartCount}`}
              animate={cartCount > 0 ? { scale: [1, 1.2, 1] } : {}}
              transition={{ duration: 0.3 }}
              className="w-8 h-8 rounded-full border-2 border-transparent hover:bg-slate-100 flex items-center justify-center transition-colors"
            >
              <ShoppingCart className="w-5 h-5 text-slate-700" />
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-600 text-white text-[10px] flex items-center justify-center rounded-full font-black z-10 shadow-sm leading-none">{cartCount}</span>
              )}
            </motion.div>
          </button>
          ...
          {user ? (
            ...
            <button onClick={onLoginClick} className="px-4 py-2 bg-white text-[#1E5EF3] hover:bg-blue-50 rounded-full transition-all flex items-center gap-2 group font-black text-sm ml-2 shadow-sm active:scale-95 leading-none">
